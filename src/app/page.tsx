...
import { AlertTriangle, CalendarDays, CheckCircle2, CircleUserRound, Clock3, LogIn, Moon, Pencil, Plus, RefreshCw, Search, Settings, Sun, Trash2, XCircle } from "lucide-react";
...
function formatExpiry(value?: string | null) {
  ...
  const remainingMs = new Date(value).getTime() - Date.now();
  if (remainingMs <= 0) return `Đã hết hạn · ${formatDate(value)}`;
  const days = Math.ceil(remainingMs / 86_400_000);
  return `${days} ngày còn lại · ${formatDate(value)}`;
}
function formatWindowDuration(minutes?: number) {
  if (!minutes) return "";
  if (minutes % 10_080 === 0) return `${minutes / 10_080} tuần`;
...
export default function Home() {
  ...
  const editExpirations = async (account: AccountView) => {
    const planDate = window.prompt("Ngày hết hạn gói (YYYY-MM-DD). Để trống nếu không biết:", account.planExpiresAt?.slice(0, 10) ?? "");
    if (planDate === null) return;
    const resetDate = window.prompt("Hạn sử dụng reset credits (YYYY-MM-DD). Để trống nếu không biết:", account.resetCreditsExpiresAt?.slice(0, 10) ?? "");
    if (resetDate === null) return;
    const toIso = (value: string) => {
      if (!value.trim()) return null;
      if (!/^\d{4}-\d{2}-\d{2}$/.test(value.trim())) throw new Error("Ngày phải có định dạng YYYY-MM-DD.");
    ...
    let resetCreditsExpiresAt: string | null;
    try { planExpiresAt = toIso(planDate); resetCreditsExpiresAt = toIso(resetDate); }
    catch (error) { setNotice(error instanceof Error ? error.message : "Ngày không hợp lệ."); return; }
    await run(account.id, async () => { await api(`/api/accounts/${account.id}`, { method: "PATCH", body: JSON.stringify({ planExpiresAt, resetCreditsExpiresAt }) }); });
  };
...
function AccountCard({ account, busy, onRefresh, onLogin, onEditExpirations, onDelete }: { account: AccountView; busy: boolean; onRefresh: () => void; onLogin: () => void; onEditExpirations: () => void; onDelete: () => void }) {
  const snapshot = account.latestSnapshot;
  const windows = visibleQuotaWindows(account);
  return <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
    <div className="flex items-start justify-between gap-3"><div><h2 className="font-bold">{account.displayName}</h2><p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">{account.maskedEmail ?? "Chưa đăng nhập"}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusColor[account.status]}`}>{statusLabel[account.status]}</span></div>
    <div className="mt-5 flex items-center justify-between border-b border-slate-100 pb-4 dark:border-slate-700"><span className="text-sm text-slate-500 dark:text-slate-400">Gói hiện tại</span><span className="rounded-lg bg-slate-100 px-2.5 py-1 text-sm font-semibold uppercase dark:bg-slate-800">{account.plan ?? "Chưa có dữ liệu"}</span></div>
    <div className="mt-4 grid gap-2 rounded-xl bg-slate-50 p-3 text-xs dark:bg-slate-800/70">
      <div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-1.5 text-slate-500 dark:text-slate-400"><CalendarDays size={14} /> Hạn gói (tự nhập)</span><span className="font-semibold">{formatExpiry(account.planExpiresAt)}</span></div>
