...
function accountView(account: any): AccountView {
  const latest = account.status === "ACTIVE"
    ? account.snapshots?.find((snapshot: any) => snapshot.status === "AVAILABLE") ?? account.snapshots?.[0]
    : account.snapshots?.[0];
...
const accountInclude = {
  snapshots: { orderBy: { checkedAt: "desc" as const }, take: 20, include: { windows: true, tokenUsage: true } },
...
export async function refreshAccount(id: string, options: { weeklyOnly?: boolean } = {}) {
  ...
  let retryCount = 0;
  const isTransientSnapshot = (value: CodexUsageSnapshot) => {
    if (value.status !== "error") return false;
    const message = (value.message ?? "").toLowerCase();
    return message.includes("timed out") || message.includes("exited while request") || message.includes("econnreset") || message.includes("network");
  };
  try {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      snapshot = await fetchCodexUsage(id, account.codexHomeId, options);
      if (!isTransientSnapshot(snapshot) || attempt === 2) break;
      retryCount += 1;
  ...
  if (isTransientSnapshot(snapshot) && account.lastSuccessAt) {
    await prisma.$transaction([
      prisma.checkAttempt.create({ data: { accountId: id, source: "CODEX_APP_SERVER", startedAt, finishedAt: new Date(), status, retryCount, errorCode: snapshot.status, errorMessage: snapshot.message } }),
      prisma.account.update({ where: { id }, data: { status: "ACTIVE", lastErrorCode: null, lastErrorMessage: null } }),
    ]);
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.checkAttempt.create({ data: { accountId: id, source: "CODEX_APP_SERVER", startedAt, finishedAt: new Date(), status, retryCount, errorCode: snapshot.status === "available" ? null : snapshot.status, errorMessage: snapshot.message } });
  });
  return getAccount(id);
}
